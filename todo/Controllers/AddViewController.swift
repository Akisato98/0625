...
class AddViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if let _selectedTask = self.selectedTask,
            let _coordinate = _selectedTask.coordinate {
            self.title = "編集"
...
//    }
    ...
    @IBAction func didTouchSaveButton(_ sender: Any) {
        ...
        if let _selectedTask = self.selectedTask {
            ...
            _selectedTask.coordinate = GeoPoint(latitude: coordinate.latitude, longitude: coordinate.longitude)
            _selectedTask.updateAt = Timestamp(date: Date())
            self.taskCollection.editTask(_selectedTask)
        } else {
            //新規のフロー
            let newTask = taskCollection.createTask()
            newTask.title = title
            newTask.memo = memoTextView.text
            newTask.coordinate = GeoPoint(latitude: coordinate.latitude, longitude: coordinate.longitude)
